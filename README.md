...
- `model` → `ANTHROPIC_MODEL`. Prefer an alias (`sonnet`, `opus`, `fable`, `haiku`) or an ID containing a known model name (e.g. `my-gateway/claude-opus-5`): Claude Code only enables auto mode, effort levels, and thinking for models it recognizes. An opaque ID like `glm-4.7` is automatically pinned behind the first free family alias, so the wire ID is unchanged while the session keeps a recognized model identity. When `model` is omitted but a pin exists, `ANTHROPIC_MODEL` is set to the best pinned alias (sonnet → opus → fable → haiku) so the session identity doesn't depend on the account default.
- `classifier_via` → redirects the auto-mode classifier to a cheaper model. The classifier always requests `sonnet`, so this fills `ANTHROPIC_DEFAULT_SONNET_MODEL` with the resolved target: `fast`/`haiku` (your haiku or fast model), `opus`/`fable`, or a literal provider ID. An explicit `models.sonnet`/`sonnet` wins. When sonnet is repurposed this way and `models.opus` is set, a `model`-less env defaults the session to `opus`. Note the classifier is the safety gate behind auto mode — pointing it at a weak model degrades both detection and can turn failures into hard blocks.
